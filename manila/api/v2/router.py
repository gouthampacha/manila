...
class APIRouter(manila.api.openstack.APIRouter):
    ...
    def _setup_routes(self, mapper):
        ...
        for path_prefix in ['/{project_id}', '']:
            # project_id is optional
            mapper.connect("snapshot_metadata",
        ...
        for path_prefix in ['/{project_id}', '']:
            ...
            mapper.connect("snapshots_export_location_metadata",
                           "%s/snapshots/{snapshot_id}"
                           "/export_locations/{resource_id}/metadata"
                           % path_prefix,
                           controller=self.resources[
                               "share_snapshot_export_locations"],
                           action="create_metadata",
            ...
            mapper.connect("snapshots_export_location_metadata",
                           "%s/snapshots/{snapshot_id}"
                           "/export_locations/{resource_id}/metadata/{key}"
                           % path_prefix,
                           controller=self.resources[
                               "share_snapshot_export_locations"],
                           action="show_metadata",
        ...
        for path_prefix in ['/{project_id}', '']:
            # project_id is optional
            mapper.connect("security_service_metadata",
                           "%s/security-services/{resource_id}/metadata"
                           % path_prefix,
                           controller=self.resources["security-services"],
                           action="create_metadata",
                           conditions={"method": ["POST"]})
            mapper.connect("security_service_metadata",
                           "%s/security-services/{resource_id}/metadata"
            ...
            mapper.connect("security_service_metadata",
                           ...
                           conditions={"method": ["GET"]})
            mapper.connect("security_service_metadata",
                           "%s/security-services/{resource_id}/metadata/{key}"
                           % path_prefix,
                           controller=self.resources["security-services"],
                           action="show_metadata",
                           conditions={"method": ["GET"]})
        ...
        for path_prefix in ['/{project_id}', '']:
            # project_id is optional
            mapper.connect("share-networks-metadata",
                           "%s/share-networks/{resource_id}/metadata"
                           % path_prefix,
                           controller=self.resources["share_networks"],
                           action="create_metadata",
                           conditions={"method": ["POST"]})
            ...
            mapper.connect("share-networks-metadata",
                           ...
                           conditions={"method": ["GET"]})
            mapper.connect("share-networks-metadata",
                           "%s/share-networks/{resource_id}/metadata/{key}"
                           % path_prefix,
                           controller=self.resources["share_networks"],
                           action="show_metadata",
                           conditions={"method": ["GET"]})
        ...
        for path_prefix in ['/{project_id}', '']:
            # project_id is optional
            mapper.connect("share-network-subnets-metadata",
                           "%s/share-networks/{share_network_id}"
                           "/subnets/{resource_id}/metadata"
                           % path_prefix,
                           controller=self.resources["share_network_subnets"],
                           action="create_metadata",
                           conditions={"method": ["POST"]})
            ...
            mapper.connect("share-network-subnets-metadata",
                           "%s/share-networks/{share_network_id}"
                           "/subnets/{resource_id}/metadata/{key}"
                           % path_prefix,
                           controller=self.resources["share_network_subnets"],
                           action="show_metadata",
                           conditions={"method": ["GET"]})
        ...
        for path_prefix in ['/{project_id}', '']:
            # project_id is optional
            mapper.connect("share-groups-metadata",
                           "%s/share-groups/{resource_id}/metadata"
                           % path_prefix,
                           controller=self.resources["share-groups"],
                           action="create_metadata",
                           conditions={"method": ["POST"]})
            ...
            mapper.connect("share-groups-metadata",
                           ...
                           conditions={"method": ["GET"]})
            mapper.connect("share-groups-metadata",
                           "%s/share-groups/{resource_id}/metadata/{key}"
                           % path_prefix,
                           controller=self.resources["share-groups"],
                           action="show_metadata",
                           conditions={"method": ["GET"]})
        ...
        for path_prefix in ['/{project_id}', '']:
            # project_id is optional
            mapper.connect("share-group-snapshots-metadata",
                           "%s/share-group-snapshots/{resource_id}/metadata"
                           % path_prefix,
                           controller=self.resources["share-group-snapshots"],
                           action="create_metadata",
                           conditions={"method": ["POST"]})
        ...
        for path_prefix in ['/{project_id}', '']:
            # project_id is optional
            mapper.connect("access-metadata",
                           "%s/share-access-rules/{resource_id}/metadata"
                           % path_prefix,
                           controller=self.resources["share-access-rules"],
                           action="create_metadata",
                           conditions={"method": ["POST"]})
